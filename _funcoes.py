...
def busca_capitulos(id, manga_link, fonte):
    print('busca_capitulos', manga_link, fonte, sep='|')
...
def zipa1(id, lista, manga, capitulo):
    ...
    path = [os.path.join(p, file) for p, _, files in os.walk(
        os.path.abspath(f'Download{divisor}{id}')) for file in files]
    ...
    for i in lista:
        ...
        if total > 41943040:  # limita o zip a 40mb
            ...
            listaTmp = []
            listaTmp.append(
                f"{str(i.split(divisor)[-3])}{divisor}{str(i.split(divisor)[-2])}{divisor}{str(i.split(divisor)[-1])}")
        else:
            total += size
            listaTmp.append(
                f"{str(i.split(divisor)[-3])}{divisor}{str(i.split(divisor)[-2])}{divisor}{str(i.split(divisor)[-1])}")
    ...
    contador = 0
    for i in listaZip:
        nomeManga = i[0].split(divisor)[-1].split('_')[0]

        fantasy_zip = zipfile.ZipFile(
            f'{folder}{id}{divisor}{manga}_capitulo_{capitulo}_parte_{contador}.zip', 'w')
        for x in i:
            dados = x.split(divisor)
            pasta = f'{dados[0]}{divisor}{dados[1]}'
            file = dados[-1]
            if file.endswith('.jpg'):
                fantasy_zip.write(os.path.join(pasta, file), os.path.relpath(
                    os.path.join(pasta, file), f'Download{divisor}{id}'), compress_type=zipfile.ZIP_DEFLATED)
        fantasy_zip.close()
        os.rename(f'{folder}{id}{divisor}{manga}_capitulo_{capitulo}_parte_{contador}.zip',
                  f'{folder}{id}{divisor}{manga}_capitulo_{capitulo}_parte_{contador}.cbr')
        shutil.copyfile(f'{folder}{id}{divisor}{manga}_capitulo_{capitulo}_parte_{contador}.cbr',
                        f'Baixados{divisor}{manga}_capitulo_{capitulo}_parte_{contador}.cbr')
        nomesZip.append(
            f'{folder}{id}{divisor}{manga}_capitulo_{capitulo}_parte_{contador}.cbr')
        contador += 1
...
if __name__ == "__main__":
    ...
    # print(busca_manga('101010', 'mashle'))
    # print(busca_capitulos(
    #     '101010', 'https://unionleitor.top/manga/mashle-magic-and-muscles', 'union'))
    # print(busca_capitulos(
