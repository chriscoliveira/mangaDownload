...
def exibeServidores(message):
    ...
    with open('logManga.log', 'a') as logger:
        logger.write(
            f'\nINFO: {message.chat.id} - {message.from_user.first_name} - Pesquisa manga: {manga}')
...
def baixarManga(message):
    ...
    with open('logManga.log', 'a') as logger:
        logger.write(
            f'\nINFO: {message.chat.id} - {message.from_user.first_name} - Download manga: {manga}, capitulos {capitulo} em {servidor}')
    ...
    if calculo <= (numeroMaximoDownload-1):
        ...
        try:
            ...
            for i in quantidadeCBR:
                ...
                try:
                    ...
                    bot.send_document(chat_id, document=open(i, 'rb'))
            ...
            with open('logManga.log', 'a') as logger:
                logger.write(
                    f'\nINFO: {message.chat.id} - {message.from_user.first_name} - Arquivo enviado {manga}_capitulo_{capitulo}_{i}.cbr')

        except Exception as e:
            print(e)
            # grava o log
            with open('logManga.log', 'a') as logger:
                logger.write(f'\nERROR: Erro ao enviar o arquivo: {e}')
            markup = types.ReplyKeyboardMarkup(one_time_keyboard=True)
            markup.add('/baixar')
            bot.send_message(
        ...
        with open('logManga.log', 'a') as logger:
            logger.write(
                f'\nWARNING: {message.chat.id} - {message.from_user.first_name} - Numero de capitulos a serem baixados excede o limite: {numeroMaximoDownload}')
        markup = types.ReplyKeyboardMarkup(one_time_keyboard=True)
        markup.add('/baixar')
        msg = bot.reply_to(
